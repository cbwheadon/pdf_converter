...
Worker.prototype._createThumbnails = function(localPath, job, callback) {
    ...
    work.push(function(done) {
	  ...
	  thumbnailer.execute(job, localPath, function(err, convertedImagePath) {
        ...
        var qr=decodeQR(convertedImagePath, _this.files);
		    console.log('convertedImagePath', convertedImagePath, _this.files);
        _this._sendReply(job.id, job.queue ,_this.files);
		    done();
	    }
...
/** Decode QR **/
decodeQR=function(dirname, files) {
  var Canvas = require('canvas')
      , Image = Canvas.Image
      , qrcode = require('jsqrcode')(Canvas)

    var filename = dirname + files[0];
 ...
 */
Worker.prototype._sendReply = function(id, queue, files){
    console.log("sending reply id: ",id, " queue:", queue, " files:" ,files);
    if(queue=='dev') {
	console.log("sending back to dev queue");
	config.set('sqsReplyUrl', this.sqs.endpoint.protocol + '//' + this.sqs.endpoint.hostname + '/dev_' + config.get('sqsReply'));
    } else {
	console.log("sending back to live queue");
	config.set('sqsReplyUrl', this.sqs.endpoint.protocol + '//' + this.sqs.endpoint.hostname + '/' + config.get('sqsReply'));
    }
